...
import { dailyTargetAtom, reverseUnitConverter, unitConverter, unitsAtom } from "@/atom/user";
...
export default function AdjustTarget() {
    ...
    const [textToDisplay, setText] = useState<string>(unitConverter(currentDailyTarget, unit).toPrecision(precisionVal));
    ...
    return (
        <View className="flex flex-col gap-5 w-full">
            <Text className="text-center text-2xl py-6 font-semibold">
                Your daily target is {unitConverter(currentDailyTarget, unit).toPrecision(precisionVal)} {unit}
            </Text>
            <View className="flex-col px-8 pt-6 gap-6">
                ...
                <StyledButton
                    ...
                    onPress={() => {
                        setCurrentDailyTarget(reverseUnitConverter(value, unit));
                        updateUserTargetAtom({newValue: Math.floor(reverseUnitConverter(value, unit))});
                    }}
                />
            </View>
        </View>
