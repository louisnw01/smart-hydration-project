...
class SmartHydrationSession:
    async def __aenter__(self):
        ...
        async with self.session.get('***REDACTED***') as response:
            ...
            async with self.session.post('***REDACTED***', data={
                '_token': token,
...
async def query(session: aiohttp.ClientSession, endpoint):
    async with session.get(f'***REDACTED***', headers=headers) as response:
